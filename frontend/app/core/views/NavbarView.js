...
define([
  ...
  var NavbarView = View.extend({
    ...
    events: {
      ...
      'click .navbar-search-empty': function()
      {
  ...
  NavbarView.prototype.cacheNavItem = function(navItemEl)
  {
    ...
    else if ($navItem.hasClass('dropdown'))
    {
      ...
      {
        var override = this.dataset.navOverride === '1';

        view.getNavItemKeysFromLi(this).forEach(function(key)
        {
          if (!view.navItems[key] || override)
          {
            view.navItems[key] = $navItem;
          }
        });
  ...
  NavbarView.prototype.showNoSearchResults = function(searchPhrase)
  {
    ...
    var cls = 'navbar-search-empty';
    var text = this.t('NAVBAR:SEARCH:' + (searchPhrase === '' ? 'help' : 'empty'));

    if (hasCamera())
    {
      cls += ' has-camera';
      text = '<i class="fa fa-qrcode"></i><span>' + text + '</span>';
    }

    this.$id('searchResults').html('<li><a class="' + cls + '">' + text + '</a></li>');
  };
  ...
  NavbarView.prototype.showBarcodeScanner = function(options)
  {
    if (!hasCamera())
    {
      return;
    ...
    var dialogView = new BarcodeScannerView();

    viewport.showDialog(dialogView);

    this.listenToOnce(dialogView, 'success', this.onBarcodeScanSuccess.bind(this, options || {}));
  };

  NavbarView.prototype.onBarcodeScanSuccess = function(options, decodedText)
  {
    var tests = [
      /[A-Z0-9]{4}\.([0-9]{9})\.[0-9]{4}/,
      /([0-9]{15})/,
      /([0-9]{12})/,
      /([0-9]+)/
    ];
    ...
    {
      var re = tests[i];
      var match = decodedText.match(re);

      if (match)
      {
        viewport.closeDialog();

        this.$id('searchPhrase').val(match[1]);

        if (window.innerWidth <= 800)
        {
          if (/^[0-9]{9}$/.test(match[1]))
          {
