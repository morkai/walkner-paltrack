...
define([
  ...
  var AUTOLINK_ID_RE = /(.)??([a-f0-9]{24}|#[0-9]+|[0-9]{15}|0*[0-9]{12}|[0-9]{9}|[A-Z]{3}[0-9]{3}[A-Z]{1})(.)??/g;
  ...
  function createAutolinker()
  {
    autolinker = new Autolinker({
      ...
      replaceFn: function(match)
      {
        ...
        tag.innerHTML = tag.innerHTML.replace(/([a-f0-9]{3})[a-f0-9]{18}([a-f0-9]{3})/, '$1...$2');
  ...
  return {
    ...
    autolinkMessage: function(text, options)
    {
      ...
      text = text.replace(AUTOLINK_ID_RE, function(match, prefix, id, suffix, offset, text)
      {
        var type;
        ...
        var aRe = new RegExp('<a href=".*?' + util.escapeRegExp(match) + '.*?"');
